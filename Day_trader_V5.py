...
def main(amount_of_coins):
    ...
    url = f'https://www.alphavantage.co/query?function=CRYPTO_INTRADAY&symbol={sys.argv[1]}&market=CAD&interval=1min&apikey={key}'
    r = requests.get(url)

    data = json.loads(r.text)
    data = json.dumps(data["Time Series Crypto (1min)"], indent=2)
    ...
    return rsi, macd_data, signal, data
...
while True:
    ...
    if (RSI > 53) and (MACD > Signal) and (MACD > 0 ) and (Signal > 0):
        buy(amount)
